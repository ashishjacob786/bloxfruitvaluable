...
function LoginForm() {
  ...
  const handleLogin = async () => {
    setLoading(true);
    // Redirect to NextAuth Roblox provider with dynamic callback URL
...
export default function LoginPage() {
  return (
    <div className="min-h-[80vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <Suspense fallback={<div className="flex justify-center"><Loader2 className="w-8 h-8 animate-spin text-blue-500" /></div>}>
        <LoginForm />
      </Suspense>
    </div>
  );
